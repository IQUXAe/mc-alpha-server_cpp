...
pub fn inventory_max_stack_size(item_id: i32) -> i32 {
    if item_id <= 0 {
        return 0;
    }
...
pub fn armor_max_damage(item_id: i32) -> i32 {
    match item_id {
...
pub fn armor_base_points(item_id: i32) -> i32 {
    match item_id {
...
pub fn inventory_add_item(
    ...
    let max_stack = inventory_max_stack_size(incoming.item_id);
...
pub fn inventory_calc_armor(armor: &[ItemStack]) -> i32 {
    ...
    for item in armor {
        ...
        let base_points = armor_base_points(item.item_id);
        if base_points <= 0 {
            continue;
        }

        let max_damage = armor_max_damage(item.item_id);
        if max_damage <= 0 {
            continue;
        }
...
/// Damages all worn armor pieces, breaking depleted ones.
pub fn inventory_damage_armor(armor: &mut [ItemStack], damage_amount: i32) {
    if damage_amount <= 0 {
        return;
    }
...
mod tests {
    ...
    fn test_food_does_not_stack() {
        ...
        for id in [260, 282, 297, 319, 320, 322, 349, 350] {
            assert_eq!(inventory_max_stack_size(id), 1, "food {id}");
        }
        assert_eq!(inventory_max_stack_size(1), 64);
        assert_eq!(inventory_max_stack_size(332), 16);
        assert_eq!(inventory_max_stack_size(267), 1);
    }
}
