...
pub fn item_furnace_facing(yaw: f32) -> u8 {
...
pub fn item_sign_yaw_meta(yaw: f32) -> u8 {
...
pub fn item_food_bite(count: i32, heal_amount: i32) -> FoodBite {
    if count > 0 {
...
mod tests {
    ...
    fn test_furnace_facing_cardinals() {
        ...
        assert_eq!(item_furnace_facing(0.0), 2);
        assert_eq!(item_furnace_facing(90.0), 5);
        assert_eq!(item_furnace_facing(180.0), 3);
        assert_eq!(item_furnace_facing(270.0), 4);
        assert_eq!(item_furnace_facing(360.0), 2);
        assert_eq!(item_furnace_facing(-90.0), 4);
    }

    #[test]
    fn test_sign_yaw_meta_range() {
        for deg in (-720..=720).step_by(15) {
            let m = item_sign_yaw_meta(deg as f32);
            assert!(m < 16, "{deg} -> {m}");
        }
        assert_eq!(item_sign_yaw_meta(0.0), 8);
        assert_eq!(item_sign_yaw_meta(180.0), 0);
    }
