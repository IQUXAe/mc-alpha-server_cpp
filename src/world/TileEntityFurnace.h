...
public:
    ...
    ItemStack* decrStackSize(int slot, int amount) override {
        ...
        if (!RustBridge::tileSlotTake(state_.slots, FURNACE_SIZE, slot, amount, &taken)
            || !taken.has_item) {
            return nullptr;
        }
        markDirty();
    ...
    void setInventorySlotContents(int slot, ItemStack* stack) override {
        ...
        bool dirty;
        if (stack) {
            dirty = RustBridge::tileSlotStore(state_.slots, FURNACE_SIZE, slot, true,
                                              stack->itemID, stack->stackSize, stack->itemDamage,
                                              getInventoryStackLimit());
        } else {
            dirty = RustBridge::tileSlotStore(state_.slots, FURNACE_SIZE, slot, false,
    ...
    void readFromNBT(const NBTCompound& nbt) override {
        ...
        // Reset state
        RustBridge::tileSlotsClear(state_.slots, FURNACE_SIZE);
